...
def main():
    ...
    x = np.zeros((10, len(T)))      # state
    xr = np.zeros((10, len(T)))     # reference state
    xt = np.zeros((9, len(T)))      # state error
    u = np.zeros((4, len(T)))
    ur = np.ones((4, len(T))) * ur0
    ut = np.zeros((4, len(T)))      # input error
...
def mpc_control(x0, u0, xt0, ur0, dt, N):
    ...
    for i in range(N):
        ...
        constr += [ur0[:,0] - ut[:,i] <= max_u.flatten()]
        constr += [ur0[:,0] - ut[:,i] >= min_u.flatten()]
    constr += [xt[:,0] == xt0.flatten()]
